...
function say(text,mood){calloutEl.textContent=text;calloutEl.classList.remove("coop-callout-glad","coop-callout-sad");if(mood==="glad"){calloutEl.classList.add("coop-callout-glad-8003d5b4");}else if(mood==="sad"){calloutEl.classList.add("coop-callout-sad-8003d5b4");}}
function showBanner(text,won){bannerEl.textContent=text;bannerEl.className="coop-banner-8003d5b4"+
(won?"coop-banner-win":"coop-banner-lose");bannerEl.hidden=false;}
function hideBanner(){bannerEl.hidden=true;bannerEl.textContent="";}
function buildRoad(){stripEl.innerHTML="";dotsEl.innerHTML="";laneTiles=[];dotEls=[];curbEl=document.createElement("div");curbEl.className="coop-lane-8003d5b4 coop-curb-8003d5b4";var curbTag=document.createElement("span");curbTag.className="coop-lane-mult-8003d5b4";curbTag.textContent="Start";curbEl.appendChild(curbTag);stripEl.appendChild(curbEl);for(var i=0;i<LANE_TOTAL;i+=1){var tile=document.createElement("div");tile.className="coop-lane-8003d5b4";var tag=document.createElement("span");tag.className="coop-lane-mult-8003d5b4";tag.textContent=LADDER[i].toFixed(2)+"×";tile.appendChild(tag);stripEl.appendChild(tile);laneTiles.push(tile);var dot=document.createElement("li");dot.className="coop-dot-8003d5b4";dotsEl.appendChild(dot);dotEls.push(dot);}
henEl=document.createElement("span");henEl.className="coop-hen-8003d5b4";henEl.setAttribute("aria-hidden","true");henEl.textContent="🐔";curbEl.appendChild(henEl);}
function clearRoad(){for(var i=0;i<LANE_TOTAL;i+=1){var tile=laneTiles[i];tile.classList.remove("coop-lane-clear","coop-lane-hit");var extras=tile.querySelectorAll(".coop-car-8003d5b4, .coop-feather-8003d5b4");for(var k=0;k<extras.length;k+=1){tile.removeChild(extras[k]);}
dotEls[i].classList.remove("coop-dot-done-8003d5b4");}
...
attemptHop();}
function attemptHop(){run.busy=true;setMoves();var target=run.lane;var tile=laneTiles[target];tile.appendChild(henEl);henEl.classList.remove("coop-hen-hop-8003d5b4");void henEl.offsetWidth;henEl.classList.add("coop-hen-hop-8003d5b4");centerTile(tile);var lucky=Math.random()<surviveOdds(target);window.setTimeout(function(){resolveHop(target,lucky);},HOP_MS);}
function resolveHop(target,lucky){if(!lucky){crash(target);return;}
run.lane=target+1;laneTiles[target].classList.add("coop-lane-clear-8003d5b4");paintDots();paintGauges();if(run.lane===LANE_TOTAL){settle(true,true);return;}
run.busy=false;setMoves();say("Lane "+run.lane+" cleared — "+
multAt(run.lane).toFixed(2)+"× is yours if you stop now.","glad");}
function crash(target){var tile=laneTiles[target];tile.classList.add("coop-lane-hit-8003d5b4");var car=document.createElement("span");car.className="coop-car-8003d5b4";car.setAttribute("aria-hidden","true");car.textContent=TRAFFIC[Math.floor(Math.random()*TRAFFIC.length)];tile.appendChild(car);henEl.classList.add("coop-hen-bonk-8003d5b4");for(var i=0;i<3;i+=1){var feather=document.createElement("span");feather.className="coop-feather-8003d5b4";feather.setAttribute("aria-hidden","true");feather.style.left=(18+i*18)+"px";feather.style.animationDelay=(i*0.12)+"s";tile.appendChild(feather);}
